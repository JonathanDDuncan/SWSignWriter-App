...
//    if (ssw.is(key,'limb')) {color = '000000';}
  ...
  canvas: function(text,options){
    ...
    var canvas = createHiDPICanvas(w ? w : 1, h ? h : 1, 1.5);
...
//      context.fillRect(0,0,w,h);
  ...
  paragraph: function (signtext, imagetype){
    ...
    var signs = (ssw.parse(signtext,chars,true).all||[]).map(function(fsw) {
      ...
      var imagetag =  ssw.svg(fsw,{copy:chars});
      if (imagetype == "png" ) imagetag =  '<img src="' + ssw.png(fsw,{copy:chars}) + '"/>';
      return '<div class="sign" style="' + style + '">' + imagetag+ '</div>';
    });
